# -*- coding: utf-8 -*-
# @FileName: train_pairs_1to3.py
# @Time : 2025/12/28 下午4:46
# @Software: PyCharm

"""
【用途】
针对 228×228×228 的 7 类沉积地层数据，训练 1→3 厚度扩展 cGAN：
- GT: ./dataset/diceng_228_228_228_zyx_change_xiangsu.npy
- 属性值: 1,2,3,4,5,6,7；0 只作为条件未知值 unknown_val 使用，GT 中不能包含 0
- 输入: 中心 1 层已知，前后两侧未知；输出: 3 层 slab
- 后续 3→5、5→7、...、23→25 建议沿用本脚本中的超参数，只修改 D、base_npz、输出目录和对应 train 文件名。

【核心修改思路】
本版 v12-stableGAN 是根据你最新一次 1→3 训练日志继续微调：
- E001→E013 属于纯监督快速收敛阶段，val_uCE 从约 0.1206 降到约 0.0034；
- E014 开始 GAN 后，val_uCE 短暂波动，但 E022 以后继续缓慢下降到约 0.0023；
- E031 以后 val_uCE 基本进入平台区，继续训练收益很小；
- dReal 在 v10 中已明显比 v9 稳定，但后期仍偶尔低于 -0.40。
因此本版保留 GAN 前提，但不再继续增强对抗项，而是采用“弱判别器 + 延后 GAN + 低频 D 更新”的稳定参数：
让 1→3 阶段先以监督项和 gDice 学稳层面几何，再用轻量 GAN 做真实性校形，避免 v11 中 E31 后 val_uCE 平台期和 dReal 偶发过负带来的覆盖风险。
1) 仍然保留 GAN 前提，不关闭 use_gan；但不再使用过早、过强的判别器。
   对 7 类层状地层，边界主要表现为连续层面起伏，若 adv 太早/太强，容易出现局部斑点、
   层界拉偏或不同阶段级联时的接缝漂移。
2) 保留不确定性，但采用“温和训练随机性 + 推理采样随机性”的组合：
   - 训练阶段：bottleneck latent noise + MC Dropout；
   - 推理阶段：生成脚本中开启 mc_dropout_inference，并使用 sampling_mode=sample。
3) best.pt 以 val_uCE 最小为主，保证几何位置稳定；
   best_gan.pt 在 val_uCE 不明显劣化的条件下保存，更适合后续论文中展示“GAN 生成效果”。
4) 多尺度策略：
   - 主尺度 128×128 仍贯穿训练，保证局部结构与层面相位稳定；
   - 辅助尺度 192×192 只做 CE+gDice，不参与 GAN，补充大尺度层面形态；
   - 根据本次日志，E25–E34 后 val_uCE 已进入平台区，因此缩短无效后期训练，
     并进一步降低 D/adv/FM 强度，避免 dReal 后期持续转负导致生成结果过度被判别器拉拽。
5) 结构感知裁剪：
   优先抽取外侧层与中心层存在差异、类别边界更丰富、类别分布不过于单一的窗口，
   避免模型把外侧未知层学习成中心层的平滑复制。

【推荐训练】
python train_pairs_1to3_diceng_v12_stableGAN.py

【输出】
- base npz:
  ./Ti/train_pairs_1to3_diceng_228_base.npz
- split json:
  ./Ti/split_pairs_fixed_test_xy_25_50_75_100_125_150_175_200_seed1234.json
- checkpoint:
  ./checkpoints_1to3_cgan_v12_stableGAN/best.pt
  ./checkpoints_1to3_cgan_v12_stableGAN/best_gan.pt

【与级联生成脚本衔接】
后续 0generate_cascade_insert10_1to3_until_23to25_in228_228_228_mult20.py 中，
建议加载 best_gan.pt；若追求最稳几何，可加载 best.pt。
多 realization 建议：
--sampling_mode sample
--sampling_temperature 1.12~1.18
--sampling_topk 5
并在 load_generator 后启用 G.mc_dropout_inference=True。
"""
...
# ============================================================
# 1) 从 GT 构造 1→3 slab 对（X + Y）并保存 npz
# ============================================================
def build_pairs_1to3_base_npz(gt_path: str, out_npz_path: str, unknown_val: int = 0):
    """
    GT: (Z,Y,X)
    X-slab target: gt[:,:,p-1:p+2] -> (3,Z,Y)
    Y-slab target: gt[:,p-1:p+2,:] -> (3,Z,X)

    cond_label: 仅中间层(索引1)已知，其余 unknown_val
    cond_mask : 仅中间层为 1，其余 0
    """
    gt = np.load(gt_path)
    ...
    x_positions = range(1, X - 1)  # 1..X-2
    y_positions = range(1, Y - 1)  # 1..Y-2
    ...
    for p in x_positions:
        slab_zyd = gt[:, :, p - 1:p + 2]
        target_dhw = np.transpose(slab_zyd, (2, 0, 1))  # (3,Z,Y)

        cond_label = np.full_like(target_dhw, unknown_val)
        cond_label[1] = gt[:, :, p]  # middle known slice
    ...
    for p in y_positions:
        slab_zdx = gt[:, p - 1:p + 2, :]
        target_dhw = np.transpose(slab_zdx, (1, 0, 2))  # (3,Z,X)
    ...
    cond_label = np.stack(cond_label_list, axis=0)  # (N,3,H,W)
    ...
    print("[OK] base npz saved:", out_npz_path,
          ...
          f"cond_label shape={cond_label.shape} (N,3,H,W)")
...
#     目标：固定 test 总数、固定 10 条指定剖面，并且保证对未来最大 PAD 也合法
# ============================================================
...
def build_consistent_split_pairs(
    ...
    生成并固化 split：
      ...
      - test 必含 fixed_test_pairs（通常 10 条）
...
def center_difference_score_dhw(arr_dhw: np.ndarray) -> float:
    """
    计算外侧层与中心已知层的差异度。
    对 1→3 来说，如果 outer 几乎只是 center 的平滑拷贝，这个分数会很低；
...
class RandomEpochDatasetMultiScale(Dataset):
    ...
    def __init__(
        ...
        use_axis: bool = True,
        crop_choices_high: Sequence[SizeHW] = ((228, 228), (216, 216), (192, 192), (160, 160)),
        seed: Optional[int] = None,
        hard_crop_num_trials: int = 24,
        hard_crop_outer_weight: float = 1.20,
        hard_crop_entropy_weight: float = 0.80,
        hard_crop_center_diff_weight: float = 0.90,
    ):
        data = np.load(npz_path, allow_pickle=True)
        self.cond_label = data["cond_label"]  # (N,3,H,W)
        ...
        self.offset = torch.tensor([-1.0, 0.0, 1.0], dtype=torch.float32)[:, None, None]  # (3,1,1)
    ...
    def __getitem__(self, _):
        ...
        cl = self.cond_label[base_idx]  # (3,H,W)
        cm = self.cond_mask[base_idx]
        yt = self.target[base_idx]
        axis_flag = int(self.meta_axis[base_idx])

        # ✅关键修复：同一 crop 坐标用于 cl/cm/yt，严禁错位
        # ✅进一步修复：不再让随机 crop 平均落到大量“简单平直层带”窗口，
        #    而是在多个候选里优先选外侧未知层边界更丰富、类别更平衡的窗口。
...
class ValDatasetFull(Dataset):
    def __init__(self, npz_path: str, val_idx: np.ndarray, K: int = 7, use_offset=True, use_axis=True):
        ...
        self.use_offset = bool(use_offset)
        self.use_axis = bool(use_axis)
        self.offset = torch.tensor([-1.0, 0.0, 1.0], dtype=torch.float32)[:, None, None]
...
class UNet2D(nn.Module):
    ...
    1) 中等偏强 GAN 训练：
       不改输入/输出维度，不破坏你后续 generate 脚本的加载方式。
    ...
    2) 生成器不确定性：
       ...
       - 推理时只有当 mc_dropout_inference=True 才会继续开启随机性。
         ...
         * 后续 generate 脚本若希望一条剖面生成多组 realization，
           只需要在加载模型后额外设置：
...
def loss_ce_known_unknown_simple(
    ...
    """
    最终统一版损失中的 CE 部分：
    1) 已知区（当前 1→3 中为中间层）使用加权 CE，保证条件严格一致；
...
# 7) 训练：3 个尺度
# ============================================================
def train_multiscale_1to3_3scales(
    npz_base_path: str,
    K: int = 7,
    epochs: int = 40,
    lr_g: float = 1.60e-4,
    ...
    save_dir: str = "./checkpoints_1to3_cgan_v12_stableGAN",
    seed: int = 1234,
):
    ...
    # 7 类沉积地层具有连续层面和多类别边界，裁剪窗口不宜过小；
    # 保留 228/216/192 等大窗口，有利于学习长波状层面起伏和类别接触关系。
    ...
    # M版进一步修正：继续保留 GAN，但把真正的衰减点后移到“局部相位先学稳”之后；
    # 同时不让 1→3 过早进入 full 主导。
    # v7-microstable 版本：在 GAN 轻量介入前后分段衰减学习率，避免后期对抗项导致 val_uCE 持续反弹。
    # v12-stableGAN：v11 日志显示增强 adv/FM 后 best_uCE 反而回升到约 0.0026；
    # 因此回调到弱 GAN 稳定版本，并把学习率衰减略后移到 [26, 36]，给监督项更多细化空间。
    ...
    def run_train_one_loader(loader, epoch, gan_enabled: bool = True):
        ...
        # 这样仍然是多尺度训练，但避免 192×192 辅助尺度上的 D/FM 把几何位置拉偏。
    ...
    def run_val(loader):
        ...
        for x2d, y_dhw, m_dhw in loader:
            ...
            unk = (m_dhw < 0.5)
            if unk.any():
                conf_u = float(max_prob[unk].mean().item())
            else:
                conf_u = 0.0
    ...
    for ep in range(1, epochs + 1):
        ...
        schedG.step()
        # 只有当本 epoch 判别器真的发生过参数更新时，才推进 D 的 scheduler。
        # 这样可以避免 adv 尚未启用阶段就先调用 lr_scheduler.step() 导致的告警，
        # 同时也避免 D 的学习率在“还没真正训练”前就被提前衰减。
        if tr_main["did_d_any"] or tr_mix["did_d_any"]:
            schedD.step()

        val = run_val(val_loader)

        # best 保存判据（overlap-stable 版）：
        # 这组日志中 uAcc 变化很小，而 val_uCE 对过拟合、模板化和对抗项接管最敏感；
        # 因此 best.pt 直接按 val_uCE 最小保存，避免后期 GAN/FM 使视觉更锐但位置更偏的模型覆盖早期最优模型。
        ...
        # GAN-priority checkpoint：给论文方法和后续可视化优先使用。
        # v12-stableGAN 继续保留 dReal 下限保护：
        # best_gan.pt 不只看 val_uCE，也要求判别器对 fake 的平均评价不能过负；
        # 这样能避免后期“几何误差微降但 GAN 状态变差”的模型覆盖更稳定的生成模型。
        gan_score = (2.00 * val["acc_unk"]) - (1.20 * val["ce_unk"]) \
                    + (0.0020 * val["disc_realism"]) + (0.0010 * val["conf_unk"])
        gan_guard_ok = (
        ...
        # 例如：scales=128x128+aux192x192 表示本 epoch 先训练 128x128 main，再训练 192x192 aux。
        ...
        # ✅打印格式：对齐 train_pairs_1to3_channel.py
        print(
        ...
        # 早停：如果 val_uCE 长期不再降低，说明继续训练只会增加模板化/接缝不一致风险。
        # 这里仍保留后期轻量 GAN 的参数，但不会让后期退化模型覆盖 best.pt。
...
if __name__ == "__main__":
    # --------------------------
    # 基础数据：7 类沉积地层，shape=(228,228,228)，GT 值域必须是 1..7
    # --------------------------
    gt_path = "./dataset/diceng_228_228_228_zyx_change_xiangsu.npy"
    base_npz = "./Ti/train_pairs_1to3_diceng_228_base.npz"

    rebuild_base = False
    if not os.path.exists(base_npz):
        rebuild_base = True
    else:
        # 若旧 base_npz 不是 D=3，自动重建，避免误用其他阶段的数据。
        try:
            _tmp = np.load(base_npz, allow_pickle=True)
            if "cond_label" not in _tmp or int(_tmp["cond_label"].shape[1]) != 3:
                rebuild_base = True
            del _tmp
        except Exception:
            rebuild_base = True
    ...
    if rebuild_base:
        ...
        build_pairs_1to3_base_npz(gt_path, base_npz, unknown_val=0)
    ...
    # --------------------------
    # 固定测试剖面：与 1→25 全部 stage 共享，永不进入训练。
    # 228 尺寸下采用 25,50,...,200，共 x/y 各 8 条，便于后续级联和不确定性分析。
    # --------------------------
    ...
    # --------------------------
    # SAFE_PAD_FOR_ALL_STAGES = 12，对应最终 23→25 的最大 PAD=12。
    # 这样同一个 split_json 可以安全复用于 1→3、3→5、...、23→25。
    # --------------------------
    SAFE_PAD_FOR_ALL_STAGES = 12
    test_ratio = 0.1
    split_seed = 1234
    ...
    if os.path.exists(split_json):
        ...
        split_info = sanitize_split_info_inplace(
        ...
        print(f"[SPLIT JSON] loaded+sanitized -> {split_json}")
    else:
    ...
    # --------------------------
    # (axis,pos) -> indices：测试集固定，训练集为剩余样本。
    # --------------------------
    ...
    # --------------------------
    # 训练：7 类层状地层推荐参数
    # 后续 3→5、5→7、...、23→25 建议保持同一组超参数，仅修改 D 与相应 base_npz/checkpoint 目录。
    # --------------------------
    train_multiscale_1to3_3scales(
        npz_base_path=base_npz,
        K=7,
        epochs=40,

        # 最新日志显示 v10 的 dReal 已稳定很多，但 best_uCE 略高于 v9；
        # 因此 v12 回调为更弱、更晚、更低频的 GAN，只保留轻量真实性校形。
        lr_g=1.60e-4,
        lr_d=7.50e-6,

        # known 保证中心剖面硬一致；gDice 处理 7 类比例不均衡；FM/adv 仅做温和真实性校形。
        lambda_known=5.8,
        lambda_gdice=0.78,
        lambda_fm=0.026,

        use_gan=True,
        aux_gan_enabled=False,       # 辅助尺度只做 CE+gDice；GAN 只作用在主尺度，降低几何漂移
        adv_start_epoch=16,
        fm_start_epoch=24,
        adv_ramp_epochs=44,
        fm_ramp_epochs=40,
        ...
        # 保留不确定性：采用温和 MC Dropout/latent noise，主要随机性留给级联生成阶段的 sample。
        mc_dropout_p=0.015,
        latent_noise_std=0.0020,

        # 228×228 数据：128 主尺度 + 192 辅助尺度；默认不启用 full 主尺度。
        epoch_size_64=3600,
        epoch_size_96=1800,
        epoch_size_full=80,
        ...
        stage64_end=40,              # 128×128 main 贯穿训练；弱 GAN 版本允许监督项继续细化到约 E36
        use_full_after=999,
        full_main_after=999,
        ...
        save_dir="./checkpoints_1to3_cgan",
        seed=split_seed,
