# -*- coding: utf-8 -*-
# @FileName: train_pairs_3to5_diceng_v12_stableGAN.py
# @Time : 2026/1/3 下午8:35
# @Software: PyCharm

"""
【用途】
针对 228×228×228 的 7 类沉积地层数据，训练 3to5 厚度扩展 cGAN：
- GT: ./dataset/diceng_228_228_228_zyx_change_xiangsu.npy
- 属性值: 1,2,3,4,5,6,7；0 只作为条件未知值 unknown_val 使用，GT 中不能包含 0。
- 输入: 中间 3 层已知，前后两侧为待生成未知层；输出: 5 层 slab。
...
【运行】
python train_pairs_3to5_diceng_v12_stableGAN.py

【输出】
- base npz: ./Ti/train_pairs_3to5_diceng_228_base.npz
- split json: ./Ti/split_pairs_fixed_test_xy_25_50_75_100_125_150_175_200_seed1234.json
- checkpoint: ./checkpoints_3to5_diceng_cgan_v12_stableGAN/best.pt 与 ./checkpoints_3to5_diceng_cgan_v12_stableGAN/best_gan.pt
"""
...
# ============================================================
# 1) 从 GT 构造 3→5 slab 对（X + Y）并保存 npz
# ============================================================
def build_pairs_3to5_base_npz(gt_path: str, out_npz_path: str, unknown_val: int = 0):
    """
    GT: (Z,Y,X)
    X-slab target: gt[:,:,p-2:p+3] -> (5,Z,Y)
    Y-slab target: gt[:,p-2:p+3,:] -> (5,Z,X)

    cond_label: 中间 3 层(索引1,2,3)已知，两侧未知层为 unknown_val
    cond_mask : 中间 3 层为 1，两侧为 0
    """
    gt = np.load(gt_path)
    ...
    x_positions = range(2, X - 2)  # 2..X-3
    y_positions = range(2, Y - 2)  # 2..Y-3
    ...
    for p in x_positions:
        slab_zyd = gt[:, :, p - 2:p + 3]
        target_dhw = np.transpose(slab_zyd, (2, 0, 1))  # (5,Z,Y)

        cond_label = np.full_like(target_dhw, unknown_val)
        cond_label[1:4] = np.transpose(gt[:, :, p - 1:p + 2], (2, 0, 1))  # middle 3 known slices
    ...
    for p in y_positions:
        slab_zdx = gt[:, p - 2:p + 3, :]
        target_dhw = np.transpose(slab_zdx, (1, 0, 2))  # (5,Z,X)

        cond_label = np.full_like(target_dhw, unknown_val)
        cond_label[1:4] = np.transpose(gt[:, p - 1:p + 2, :], (1, 0, 2))  # middle 3 known slices
    ...
    cond_label = np.stack(cond_label_list, axis=0)  # (N,5,H,W)
    ...
    print("[OK] base npz saved:", out_npz_path,
          ...
          f"cond_label shape={cond_label.shape} (N,5,H,W)")
...
def center_difference_score_dhw(arr_dhw: np.ndarray) -> float:
    """
    计算外侧层与中心已知层的差异度。
    对 3→5 来说，如果两侧 outer 仍接近中间 3 层的平滑模板，这个分数会偏低；
...
class RandomEpochDatasetMultiScale(Dataset):
    ...
    def __init__(
        ...
        self.cond_label = data["cond_label"]  # (N,5,H,W)
        ...
        self.offset = torch.tensor([-2.0, -1.0, 0.0, 1.0, 2.0], dtype=torch.float32)[:, None, None]  # (5,1,1)
...
class ValDatasetFull(Dataset):
    def __init__(self, npz_path: str, val_idx: np.ndarray, K: int = 7, use_offset=True, use_axis=True):
        ...
        self.use_offset = bool(use_offset)
        self.use_axis = bool(use_axis)
        self.offset = torch.tensor([-2.0, -1.0, 0.0, 1.0, 2.0], dtype=torch.float32)[:, None, None]
...
class UNet2D(nn.Module):
    ...
    1) 不改 3→5 的输入/输出维度，不破坏后续 generate 脚本的加载方式；
...
def loss_ce_known_unknown_simple(
    ...
    """
    最终统一版损失中的 CE 部分：
    1) 已知区（当前 3→5 中为中间 3 层）使用加权 CE，保证条件严格一致；
...
# 7) 训练：3 个尺度
# ============================================================
def train_multiscale_3to5_3scales(
    npz_base_path: str,
    K: int = 7,
    epochs: int = 40,
    lr_g: float = 1.60e-4,
    ...
    save_dir: str = "./checkpoints_3to5_diceng_cgan_v12_stableGAN",
    seed: int = 1234,
):
    ...
    for ep in range(1, epochs + 1):
        ...
        # ✅打印格式：对齐 train_pairs_3to5_channel.py
        print(
...
if __name__ == "__main__":
    ...
    base_npz = "./Ti/train_pairs_3to5_diceng_228_base.npz"

    if (not os.path.exists(base_npz)):
        os.makedirs(os.path.dirname(base_npz) or ".", exist_ok=True)
        build_pairs_3to5_base_npz(gt_path, base_npz, unknown_val=0)
    ...
    if os.path.exists(split_json):
        ...
        # 若同一个 split_json 已经由 1→3 生成，则当前 3→5 直接复用其 test_pairs_all，
        ...
        if missing_pairs:
            raise RuntimeError(
                f"已有 split_json 中的部分测试对在当前 3→5 base_npz 中不存在：{missing_pairs[:10]}\n"
        ...
        split_info = build_consistent_split_pairs(
            ...
            seed=split_seed,
            safe_pad_for_all_stages=SAFE_PAD_FOR_ALL_STAGES,
            pad_of_this_stage=2,                  # 3→5 本 stage 的 PAD=2
        )
    ...
    train_multiscale_3to5_3scales(
        ...
        gan_dreal_min=-0.40,
        save_dir="./checkpoints_3to5_cgan",
        seed=split_seed,
