# -*- coding: utf-8 -*-
# @FileName: train_pairs_5to7_diceng_v12_stableGAN.py
# @Time : 2026/1/3 下午8:35
# @Software: PyCharm

"""
【用途】
针对 228×228×228 的 7 类沉积地层数据，训练 5to7 厚度扩展 cGAN：
- GT: ./dataset/diceng_228_228_228_zyx_change_xiangsu.npy
- 属性值: 1,2,3,4,5,6,7；0 只作为条件未知值 unknown_val 使用，GT 中不能包含 0。
- 输入: 中间 5 层已知，前后两侧为待生成未知层；输出: 7 层 slab。
- 本脚本与 1→3 的 v12_stableGAN 版本保持同一套超参数、split_json 和 checkpoint 命名规则，便于后续 1→3、3→5、...、23→25 级联调用。
...
1) GAN 是前提：保留 hinge adversarial loss + Feature Matching，但将对抗项延后到第 16 轮，并采用长 ramp 和更低 lrD，避免 7 类层状地层的层界被判别器过早拉偏。
2) 监督项仍以 unknown 区 CE + unknown-only generalized Dice 为主；known 区 CE 强约束中间已知层，保证条件剖面不被破坏。
3) 保留轻量 MC Dropout 和 bottleneck latent noise：训练期不过度扰动，后续级联生成 20 组 realization 时可在 generate 脚本中开启 mc_dropout_inference 与 sample 采样来保留不确定性。
...
【运行】
python train_pairs_5to7_diceng_v12_stableGAN.py

【输出】
- base npz: ./Ti/train_pairs_5to7_diceng_228_base.npz
- split json: ./Ti/split_pairs_fixed_test_xy_25_50_75_100_125_150_175_200_seed1234.json
- checkpoint: ./checkpoints_5to7_diceng_cgan_v12_stableGAN/best.pt 与 ./checkpoints_5to7_diceng_cgan_v12_stableGAN/best_gan.pt
"""
...
# ============================================================
# 1) 从 GT 构造 5→7 slab 对（X + Y）并保存 npz
# ============================================================
def build_pairs_5to7_base_npz(gt_path: str, out_npz_path: str, unknown_val: int = 0):
    """
    GT: (Z,Y,X)
    X-slab target: gt[:,:,p-3:p+4] -> (7,Z,Y)
    Y-slab target: gt[:,p-3:p+4,:] -> (7,Z,X)

    cond_label: 中间 5 层(索引1,2,3,4,5)已知，两侧未知层为 unknown_val
    cond_mask : 中间 5 层为 1，两侧为 0
    """
    gt = np.load(gt_path)
    ...
    x_positions = range(3, X - 3)  # 3..X-4
    y_positions = range(3, Y - 3)  # 3..Y-4
    ...
    for p in x_positions:
        slab_zyd = gt[:, :, p - 3:p + 4]
        target_dhw = np.transpose(slab_zyd, (2, 0, 1))  # (7,Z,Y)

        cond_label = np.full_like(target_dhw, unknown_val)
        cond_label[1:6] = np.transpose(gt[:, :, p - 2:p + 3], (2, 0, 1))  # middle 5 known slices
    ...
    for p in y_positions:
        slab_zdx = gt[:, p - 3:p + 4, :]
        target_dhw = np.transpose(slab_zdx, (1, 0, 2))  # (7,Z,X)

        cond_label = np.full_like(target_dhw, unknown_val)
        cond_label[1:6] = np.transpose(gt[:, p - 2:p + 3, :], (1, 0, 2))  # middle 5 known slices
    ...
    cond_label = np.stack(cond_label_list, axis=0)  # (N,7,H,W)
    ...
    print("[OK] base npz saved:", out_npz_path,
          ...
          f"cond_label shape={cond_label.shape} (N,7,H,W)")
...
def center_difference_score_dhw(arr_dhw: np.ndarray) -> float:
    """
    计算外侧层与中心已知层的差异度。
    对 5→7 来说，如果两侧 outer 仍接近中间 5 层的平滑模板，这个分数会偏低；
...
class RandomEpochDatasetMultiScale(Dataset):
    ...
    def __init__(
        ...
        use_axis: bool = True,
        crop_choices_high: Sequence[SizeHW] = ((228, 228), (224, 224), (216, 216), (208, 208), (192, 192), (176, 176), (160, 160)),
        ...
        self.cond_label = data["cond_label"]  # (N,7,H,W)
        ...
        self.offset = torch.tensor([-3.0, -2.0, -1.0, 0.0, 1.0, 2.0, 3.0], dtype=torch.float32)[:, None, None]  # (7,1,1)
    ...
    def __getitem__(self, _):
        ...
        cl = self.cond_label[base_idx]  # (7,H,W)
...
class ValDatasetFull(Dataset):
    def __init__(self, npz_path: str, val_idx: np.ndarray, K: int = 7, use_offset=True, use_axis=True):
        ...
        self.use_offset = bool(use_offset)
        self.use_axis = bool(use_axis)
        self.offset = torch.tensor([-3.0, -2.0, -1.0, 0.0, 1.0, 2.0, 3.0], dtype=torch.float32)[:, None, None]
...
class UNet2D(nn.Module):
    ...
    1) 不改 5→7 的输入/输出维度，不破坏后续 generate 脚本的加载方式；
...
def loss_ce_known_unknown_simple(
    ...
    """
    最终统一版损失中的 CE 部分：
    1) 已知区（当前 5→7 中为中间 5 层）使用加权 CE，保证条件严格一致；
...
# 7) 训练：3 个尺度
# ============================================================
def train_multiscale_5to7_3scales(
    npz_base_path: str,
    K: int = 7,
    epochs: int = 40,
    lr_g: float = 1.60e-4,
    ...
    save_dir: str = "./checkpoints_5to7_diceng_cgan_v12_stableGAN",
    seed: int = 1234,
):
    ...
    for ep in range(1, epochs + 1):
        ...
        # ✅打印格式：对齐 train_pairs_5to7_channel.py
        print(
...
if __name__ == "__main__":
    ...
    base_npz = "./Ti/train_pairs_5to7_diceng_228_base.npz"

    if (not os.path.exists(base_npz)):
        os.makedirs(os.path.dirname(base_npz) or ".", exist_ok=True)
        build_pairs_5to7_base_npz(gt_path, base_npz, unknown_val=0)
    ...
    if os.path.exists(split_json):
        ...
        # 若同一个 split_json 已经由 1→3 生成，则当前 5→7 直接复用其 test_pairs_all，
        ...
        if missing_pairs:
            raise RuntimeError(
                f"已有 split_json 中的部分测试对在当前 5→7 base_npz 中不存在：{missing_pairs[:10]}\n"
        ...
        split_info = build_consistent_split_pairs(
            ...
            seed=split_seed,
            safe_pad_for_all_stages=SAFE_PAD_FOR_ALL_STAGES,
            pad_of_this_stage=3,                  # 5→7 本 stage 的 PAD=3
        )
    ...
    # 2) 根据 1→3 日志表现，GAN 不宜过早/过强介入，因此后移 adv 并降低 D 更新强度；
    # 3) 保留适度 MC Dropout + latent noise，便于后续同一剖面生成多组 realization。
    # --------------------------
    # 7 类地层 v12_stableGAN 推荐参数：
    # 与 1→3 保持一致，GAN 延后轻量介入，训练期只保留温和随机性；
    # 若显存不足，优先降低 batch_64/batch_96，不建议优先改动 lambda_adv/lambda_fm/dropout。
    # --------------------------
    train_multiscale_5to7_3scales(
        npz_base_path=base_npz,
        K=7,
        epochs=40,

        # diceng-v12-stableGAN：保留 GAN 对抗生成，
        # 但仅在 128×128 主尺度上启用 D/FM/adv；192×192 辅助尺度只做 CE+gDice 监督，减少几何漂移。
        lr_g=1.60e-4,
        lr_d=7.50e-6,
        ...
        gan_dreal_min=-0.40,
        save_dir="./checkpoints_5to7_cgan",
        seed=split_seed,
